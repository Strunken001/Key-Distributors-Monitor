// The file contents for the current environment will overwrite these during build.
// The build system defaults to the dev environment which uses `environment.ts`, but if you do
// `ng build --env=prod` then `environment.prod.ts` will be used instead.
// The list of which env maps to which file can be found in `.angular-cli.json`.

export const environment = {
  production: false,
  BASE_URL:  'https://gtweb6.gtbank.com.gtbank.com/Appservices',
  // BASE_URL:  'https://collection.gtbank.com/WEBAPIs/PubEncrypt4',
  AUTH_API: '/GTBAuthenticationService/Api',
  KDMonitor_Api: '/KDMonitorWebService/Api',
  PUB_GAPS_ENC_KEY: `MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCRA67mG4zCbKULFvEyn0UafxPN
  OO42qk3U0Qg2WOTp6l0EOz9ZiHW4fDMHrOcIlPIvwOjkHWpI1VSi1H/I6zbgkDwR
  Gu/OYQ2Eih9YeYwL9DcmbNoe8uXTIwVLtWS7OW/wtLQz+vup/vHTeIWPs0Y+LDcO
  7BcOGLli53GwqZC+3QIDAQAB`,
  CHANNEL: 'GAPS',
  KDChannel: 'KD',
  CHANNEL_SHORTNAME: 'GP',
  REFRESH_INTERVAL: 100000,
  CACHE_SIZE: 1,
  // tslint:disable-next-line:max-line-length
  PUB_ENC_KEY: `MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCRA67mG4zCbKULFvEyn0UafxPNOO42qk3U0Qg2WOTp6l0EOz9ZiHW4fDMHrOcIlPIvwOjkHWpI1VSi1H/I6zbgkDwRGu/OYQ2Eih9YeYwL9DcmbNoe8uXTIwVLtWS7OW/wtLQz+vup/vHTeIWPs0Y+LDcO7BcOGLli53GwqZC+3QIDAQAB`,
  // tslint:disable-next-line: max-line-length
  // PUB_ENC_KEY: `MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCaVYYYXZVcdf1NLYHD9a8Ast7E  9n0HVRR1tX0hh9qqCn8/yy/uPvVSFLiK+lTlZPqRiSi7WplDTNf1V/Bmd70Otzz2  Ke9sX2SCUz85zsxzsDLnAtI7irj65k11G8EqJEkOCVw9ahU6xA5xZLYKJyBqqe04  UUq0LoZ6v26xohU9uQIDAQAB`,

// tslint:disable-next-line:max-line-length --- main one for Ibank
  PRIV_ENC_KEY: `-----BEGIN RSA PRIVATE KEY-----
  
  NH84chH6XyhJ7C9RbwUOPOzYA97pDyUGhSH17oWmdr/yY9E44BnzHuBF
  -----END RSA PRIVATE KEY-----`
};
